...
def _dispatch_local(
    ...
    if is_dia(mode):
        # Auto-inject the latest Carafe library if one exists and no lib_path is set
        if not lib_path and instrument_config.get("carafe_enabled"):
            from stan.search.carafe import get_latest_library
            carafe_lib = get_latest_library(instrument_config.get("name", ""))
            if carafe_lib:
                lib_path = str(carafe_lib)
                logger.info(
                    "Carafe: injecting experiment-specific library → %s",
                    carafe_lib.name,
                )

        logger.info("Local DIA-NN search for %s", raw_path.name)
        result = run_diann_local(
            raw_path=raw_path,
        ...
        _dispatch_comparison(raw_path, mode, output_dir, instrument_config)
        return result

    if is_dda(mode):
        logger.info("Local Sage search for %s", raw_path.name)
        result = run_sage_local(
            raw_path=raw_path,
            output_dir=output_dir,
            vendor=vendor,
...
def _dispatch_slurm(
    ...
    if is_dia(mode):
        # Auto-inject latest Carafe library if available and no lib configured
        if not instrument_config.get("lib_path") and instrument_config.get("carafe_enabled"):
            from stan.search.carafe import get_latest_library
            carafe_lib = get_latest_library(instrument_config.get("name", ""))
            if carafe_lib:
                instrument_config = {**instrument_config, "lib_path": str(carafe_lib)}
                logger.info(
                    "Carafe: injecting experiment-specific library → %s",
                    carafe_lib.name,
                )
