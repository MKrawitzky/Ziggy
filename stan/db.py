...
CREATE TABLE IF NOT EXISTS runs (
    ...
    missed_cleavage_rate_2plus REAL,
    pct_charge_1    REAL,
    pct_charge_2    REAL,
    pct_charge_3    REAL,
    pct_charge_4plus REAL,
...
CREATE TABLE IF NOT EXISTS search_comparisons (
    ...
    engine      TEXT NOT NULL,   -- 'msfragger_dda' | 'msfragger_dia' | future engines
    status      TEXT NOT NULL DEFAULT 'pending',  -- 'pending' | 'running' | 'done' | 'failed'
    started_at  TEXT,
    finished_at TEXT,
    n_psms      INTEGER,         -- PSMs at E-value < 0.01 (DDA) or precursors (DIA)
    n_peptides  INTEGER,         -- unique peptide sequences
    n_proteins  INTEGER,         -- unique protein accessions
    n_precursors INTEGER,        -- DIA precursors (engine-specific)
    result_path TEXT,            -- output directory
    error_msg   TEXT,
...
def _migrate(con: sqlite3.Connection) -> None:
    ...
    migrations: list[tuple[str, str]] = [
        ...
        # TIMS ion mobility metrics from 4DFF .features (added 2026-04-11)
        ("n_4d_features", "ALTER TABLE runs ADD COLUMN n_4d_features INTEGER"),
        ("median_mobility_fwhm", "ALTER TABLE runs ADD COLUMN median_mobility_fwhm REAL"),
        ("median_rt_fwhm_features", "ALTER TABLE runs ADD COLUMN median_rt_fwhm_features REAL"),
        # Carafe2 experiment-specific library built from this run (added 2026-04-12)
        ("carafe_library", "ALTER TABLE runs ADD COLUMN carafe_library TEXT"),
        # 4DFF .features file path — populated by auto feature finding (added 2026-04-12)
        ("features_path", "ALTER TABLE runs ADD COLUMN features_path TEXT"),
        # DIA-NN / Sage result file path — for fallback mobility extraction (added 2026-04-12)
        ("result_path", "ALTER TABLE runs ADD COLUMN result_path TEXT"),
        # Missed cleavage rate for 2+ missed cleavages (added 2026-04-15)
    ...
    # Data migration: Bruker .d files were historically labelled "DIA" by an
    # older baseline builder that used AcquisitionMode.DIA_ORBITRAP as its
    # default.  Any run whose raw_path ends with ".d" (or ".d/", "\.d\", etc.)
    # is Bruker diaPASEF — fix those rows so the dashboard shows the correct label.
    fixed = con.execute(
        """
        UPDATE runs
           SET mode = 'diaPASEF'
    ...
    if fixed:
        logger.info("Data migration: corrected %d 'DIA' → 'diaPASEF' for Bruker .d files", fixed)
...
def insert_run(
    ...
    Args:
        ...
        result_path: Path to the search result file (report.parquet or
            results.sage.parquet).  Used as fallback for mobility visualization.
    ...
    row = {
        ...
        "missed_cleavage_rate_2plus": metrics.get("missed_cleavage_rate_2plus"),
        ...
        "diagnosis": diagnosis,
        # Search result path (for mobility fallback)
...
def update_features_path(run_id: str, features_path: str, db_path: Path | None = None) -> None:
    """Store the path to the 4DFF .features file for a run."""
    if db_path is None:
        db_path = get_db_path()
    with sqlite3.connect(str(db_path)) as con:
        con.execute(
            "UPDATE runs SET features_path = ? WHERE id = ?",
            (features_path, run_id),
        )
    logger.info("Run %s: features_path stored → %s", run_id[:8], features_path)
