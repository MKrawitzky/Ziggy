...
def detect_lc_system(raw_path) -> str | None:
    ...
    Returns one of "evosep", "nanoelute", "nanoelute2", "proteolute",
    "custom", or None if unknown. Used to split the community TIC overlay
    so Evosep standardized traces aren't mixed with custom nanoLC gradients
    in the same bucket.
    """
    from pathlib import Path as _Path
    import re
    ...
    if path.suffix.lower() == ".d" and path.is_dir():
        # hystar.method carries the ModuleName of the LC plugin, which is
        # the most reliable signal for distinguishing LC systems.
        # submethods.xml with HyStar_LC + "samples per day" is Evosep-specific.
            ...
            if hystar.exists():
                try:
                    ...
                    if "Evosep One" in text or "EVOSEP_ONE" in text:
                        return "evosep"
                    # nanoElute 2 must be checked before nanoElute (substring).
                    if "nanoelute 2" in tl or "nanoelute2" in tl:
                        return "nanoelute2"
                    if "bruker nanoelute" in tl:
                        return "nanoelute"
                    # "Bruker proteoElute" (lowercase p) seen in production;
                    # match case-insensitively ("proteoElute".lower() = "proteoelute").
                    if "bruker proteoelute" in tl:
                        return "proteolute"
                except OSError:
                    pass
            submeth = m_dir / "submethods.xml"
