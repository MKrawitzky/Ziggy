...
dll.tims_open_recalibration_id.argtypes = [ c_char_p, c_char_p ]
dll.tims_open_recalibration_id.restype = c_uint64
...
dll.tims_get_calibration_id.argtypes = [ c_uint64, c_char_p, c_uint32 ]
dll.tims_get_calibration_id.restype = c_uint32
...
dll.tims_read_pasef_msms_v2.argtypes = [ c_uint64, POINTER(c_int64), c_uint32, MSMS_SPECTRUM_FUNCTOR, c_void_p  ]
dll.tims_read_pasef_msms_v2.restype = c_uint32
dll.tims_read_pasef_msms_for_frame_v2.argtypes = [ c_uint64, c_int64, MSMS_SPECTRUM_FUNCTOR, c_void_p  ]
dll.tims_read_pasef_msms_for_frame_v2.restype = c_uint32
dll.tims_read_pasef_msms_v2_2024.argtypes = [ c_uint64, POINTER(c_int64), c_uint32, MSMS_SPECTRUM_FUNCTOR, c_void_p  ]
dll.tims_read_pasef_msms_v2_2024.restype = c_uint32
dll.tims_read_pasef_msms_for_frame_v2_2024.argtypes = [ c_uint64, c_int64, MSMS_SPECTRUM_FUNCTOR, c_void_p  ]
dll.tims_read_pasef_msms_for_frame_v2_2024.restype = c_uint32
...
dll.tims_extract_centroided_spectrum_for_frame_v3.argtypes = [ c_uint64, c_int64, c_uint32, c_uint32, MSMS_SPECTRUM_FUNCTOR, c_void_p ]
dll.tims_extract_centroided_spectrum_for_frame_v3.restype = c_uint32
...
dll.tims_extract_centroided_spectrum_for_frame_ext_v3.argtypes = [ c_uint64, c_int64, c_uint32, c_uint32, c_double, MSMS_SPECTRUM_FUNCTOR, c_void_p ]
dll.tims_extract_centroided_spectrum_for_frame_ext_v3.restype = c_uint32
...
class TimsData:
    def __init__ (self, analysis_directory, use_recalibrated_state=False, pressure_compensation_strategy=PressureCompensationStrategy.NoPressureCompensation, recalibration_id=None):
        if not isinstance(analysis_directory, str):
            raise ValueError("analysis_directory must be a string.")
        self.dll = dll
        
        if recalibration_id == None:
            ...
            self.handle = self.dll.tims_open_recalibration_id(
                analysis_directory.encode('utf-8'),
    ...
    @classmethod
    def fromRecalibrationId(cls, analysis_directory, recalibration_id):
    ...
    def readPasefMsMs (self, precursor_list, sparse=True):
        ...
        rc = 0
        if sparse:
            rc = self.dll.tims_read_pasef_msms_v2(self.handle,
                                               precursors_for_dll.ctypes.data_as(POINTER(c_int64)),
                                               len(precursor_list),
                                               callback_for_dll,
                                               None)
        else:
            rc = self.dll.tims_read_pasef_msms_v2_2024(self.handle,
                                               precursors_for_dll.ctypes.data_as(POINTER(c_int64)),
                                               len(precursor_list),
                                               callback_for_dll,
                                               None)
    ...
    # 'precursor_id' to a pair of arrays (mz_values, area_values).
    def readPasefMsMsForFrame (self, frame_id, sparse=True):
        result = {}
        ...
        rc = 0
        if sparse:
            rc = self.dll.tims_read_pasef_msms_for_frame_v2(self.handle,
                                                         frame_id,
                                                         callback_for_dll,
                                                         None)
        else:
            rc = self.dll.tims_read_pasef_msms_for_frame_v2_2024(self.handle,
                                                            frame_id,
                                                            callback_for_dll,
                                                            None)
        if rc == 0:
    ...
    def extractCentroidedSpectrumForFrame (self, frame_id, scan_begin, scan_end, peak_picker_resolution=None, dense=True):
        result = None
        ...
        if peak_picker_resolution is None:
            if dense:
                ...
                rc = self.dll.tims_extract_centroided_spectrum_for_frame_v3(
                    self.handle,
                    frame_id,
                    scan_begin,
                    scan_end,
            ...
            if dense:
                ...
                rc = self.dll.tims_extract_centroided_spectrum_for_frame_ext_v3(
                    self.handle,
                    frame_id,
                    scan_begin,
                    scan_end,
    ...
    def extractChromatograms (self, jobs, trace_sink):
        ...
        def wrap_gen(job, user_data):
            try:
                ...
                return 0  # no more jobs — tell DLL to stop
    ...
    def getCalibrationId(self):
        """ Return the currently used re-calibration id or None if the instrument calibration is used. """
        
        len = self.dll.tsf_get_calibration_id(None, 0)
        if len == 0:
            return None
        buf = create_string_buffer(len)
        self.dll.tsf_get_calibration_id(buf, len)
        return buf.value
