...
@app.command()
def timsplot(
    port: int = typer.Option(8422, "--port", "-p", help="timsplot port"),
    install: bool = typer.Option(False, "--install", help="Clone/update timsplot from GitHub"),
    no_browser: bool = typer.Option(False, "--no-browser", help="Don't open browser automatically"),
) -> None:
    """Launch the timsplot data analysis and visualization app.

    timsplot is a Shiny web application for interactive proteomics figure
    generation. It reads DIA-NN, Spectronaut, Sage, and FragPipe search
    reports and produces publication-ready plots.

    timsplot is installed to ~/.stan/tools/timsplot/ on first run (requires
    git and pip). Use --install to update an existing installation.
    """
    import subprocess
    import sys

    tools_dir = get_user_config_dir() / "tools" / "timsplot"
    repo_url = "https://github.com/zack-kirsch/timsplot.git"

    # ── Install / update ────────────────────────────────────────────
    if install or not tools_dir.exists():
        if not tools_dir.exists():
            console.print("[bold]timsplot not found — installing...[/bold]")
            ...
            console.print("[bold]Updating timsplot...[/bold]")
            subprocess.run(["git", "-C", str(tools_dir), "pull", "--ff-only"],
                           capture_output=True)
            console.print("  [green]Updated.[/green]")

        req_file = tools_dir / "requirements.txt"
        if req_file.exists():
            console.print("  Installing Python dependencies (this may take a minute)...")
            result = subprocess.run(
                [sys.executable, "-m", "pip", "install", "-q", "-r", str(req_file)],
                capture_output=True, text=True,
            )
            if result.returncode != 0:
                console.print(f"[yellow]pip install warnings:[/yellow] {result.stderr[-500:].strip()}")
            else:
                console.print("  [green]Dependencies installed.[/green]")

    if not tools_dir.exists():
        console.print("[red]timsplot directory not found after install attempt.[/red]")
        raise typer.Exit(1)

    app_py = tools_dir / "app.py"
    if not app_py.exists():
        console.print(f"[red]app.py not found in {tools_dir}[/red]")
        raise typer.Exit(1)

    # ── Launch ──────────────────────────────────────────────────────
    url = f"http://127.0.0.1:{port}"
    console.print(f"[bold]timsplot[/bold] — proteomics data analysis")
    console.print(f"  {url}")
    console.print(f"  Load DIA-NN report.parquet, Spectronaut .tsv, or Sage results")
    console.print()
...
@app.command("carafe")
def carafe_cmd(
    install: bool = typer.Option(False, "--install", help="Download and install Carafe v2.0.0"),
    run_id: str = typer.Option(None, "--run", "-r", help="Run ID from STAN DB to build library for"),
    fasta: Path = typer.Option(None, "--fasta", "-f", help="FASTA protein database path"),
    diann_report: Path = typer.Option(None, "--report", help="DIA-NN report.parquet path (auto-inferred if omitted)"),
    output_dir: Path = typer.Option(None, "--output", "-o", help="Output directory for library files"),
    java: str = typer.Option("java", "--java", help="Path to java executable"),
) -> None:
    """Build an experiment-specific spectral library with Carafe2.

    Carafe2 trains deep-learning models (RT, fragment intensity, ion mobility)
    directly on your DIA data and outputs a DIA-NN-compatible TSV spectral
    library tuned to your instrument and sample prep.

    Workflow:
      1. Install Carafe (once):          stan carafe --install
      2. List runs and show commands:    stan carafe
      3. Build a library for a run:      stan carafe --run <ID> --fasta db.fasta

    Reference: https://github.com/Noble-Lab/Carafe
    ...
    from stan.db import get_runs, get_run

    tools_dir = get_user_config_dir() / "tools" / "carafe"
    jar_pattern = "carafe-*.jar"
    download_url = "https://github.com/Noble-Lab/Carafe/releases/download/v2.0.0/carafe-2.0.0.zip"
    version = "2.0.0"

    def _find_jar() -> Path | None:
        """Return path to carafe JAR if installed."""
        import glob
        matches = list(tools_dir.glob("carafe-*.jar"))
        if matches:
            return matches[0]
        # Also check one level deep (common zip layout)
        matches = list(tools_dir.glob("*/carafe-*.jar"))
    ...
    if install:
        ...
        if result.returncode != 0:
            result = subprocess.run([java, "-version"], capture_output=True, text=True)
        if result.returncode != 0:
            console.print(f"[red]Java not found at '{java}'.[/red]")
            console.print("Install Java 21+ from https://adoptium.net/ and ensure it's on PATH.")
            raise typer.Exit(1)
        java_ver = (result.stdout or result.stderr).split("\n")[0]
        console.print(f"  Java: {java_ver.strip()}")

        tools_dir.mkdir(parents=True, exist_ok=True)
        zip_path = tools_dir / f"carafe-{version}.zip"

        if _find_jar():
            console.print(f"[green]Carafe already installed:[/green] {_find_jar()}")
        else:
            console.print(f"[bold]Downloading Carafe v{version}[/bold] ({download_url})")
            console.print("  207 MB — this may take a minute...")
            ...
            jar = _find_jar()
            if not jar:
                console.print("[red]JAR not found after extraction. Check the zip contents manually.[/red]")
                console.print(f"  Extracted to: {tools_dir}")
                raise typer.Exit(1)
            console.print(f"[green]Carafe installed:[/green] {jar}")

        console.print()
        console.print("Next:  [bold]stan carafe --run <RUN_ID> --fasta /path/to/db.fasta[/bold]")
        return
    ...
    if not run_id:
        ...
        d_runs = [r for r in runs if r.get("raw_path", "").endswith(".d")]

        console.print(f"[bold]Carafe v{version}[/bold] — experiment-specific library builder")
        if not jar:
            console.print(f"  [yellow]Not installed.[/yellow]  Run: [bold]stan carafe --install[/bold]")
        else:
            console.print(f"  JAR: {jar}")
        console.print()

        if not d_runs:
            console.print("[yellow]No Bruker .d runs found in STAN database.[/yellow]")
            return

        console.print(f"Available runs ({len(d_runs)}):\n")
        for r in d_runs[:20]:
            gate_color = {"pass": "green", "warn": "yellow", "fail": "red"}.get(r.get("gate_result", ""), "white")
            console.print(
                f"  [{gate_color}]{r.get('gate_result','?'):4}[/{gate_color}]  "
                f"{r['id'][:8]}…  {r['run_name'][:55]}"
            )
        if len(d_runs) > 20:
            console.print(f"  … and {len(d_runs) - 20} more")

        console.print()
        console.print("Build a library:")
        console.print("  [bold]stan carafe --run <RUN_ID> --fasta /path/to/proteome.fasta[/bold]")
        return

    # ── Run Carafe ───────────────────────────────────────────────────
    if not jar:
        console.print("[red]Carafe not installed.[/red]  Run: [bold]stan carafe --install[/bold]")
        raise typer.Exit(1)

    run = get_run(run_id)
    if not run:
        # Try prefix match
        all_runs = get_runs(limit=2000)
        matches = [r for r in all_runs if r["id"].startswith(run_id)]
        if len(matches) == 1:
            run = matches[0]
        elif len(matches) > 1:
            console.print(f"[yellow]Ambiguous prefix '{run_id}' matches {len(matches)} runs.[/yellow]")
            raise typer.Exit(1)
        else:
    ...
    if not fasta:
        console.print("[red]--fasta is required.[/red]  Provide path to protein FASTA database.")
        raise typer.Exit(1)
    if not Path(fasta).exists():
        console.print(f"[red]FASTA not found:[/red] {fasta}")
        raise typer.Exit(1)

    # Infer DIA-NN report path if not provided
    if not diann_report:
        # Common conventions: report.parquet or report.tsv alongside the .d
        for candidate in [
            raw_path.parent / "report.parquet",
            raw_path.parent / "report.tsv",
            raw_path.parent / (raw_path.stem + ".report.parquet"),
        ]:
            if candidate.exists():
                diann_report = candidate
                break

    # Output directory
    out_dir = Path(output_dir) if output_dir else raw_path.parent / f"carafe_{raw_path.stem}"
    out_dir.mkdir(parents=True, exist_ok=True)
    out_lib = out_dir / f"{raw_path.stem}_carafe.tsv"

    console.print(f"[bold]Carafe[/bold] — building experiment-specific library")
    console.print(f"  Run:    {run['run_name']}")
    console.print(f"  Input:  {raw_path}")
    console.print(f"  FASTA:  {fasta}")
    console.print(f"  Report: {diann_report or '[yellow]not found — Carafe will run library-free mode[/yellow]'}")
    console.print(f"  Output: {out_lib}")
    console.print()

    cmd = [
        java, "-jar", str(jar),
        "--input-raw", str(raw_path),
        "--organism-database", str(fasta),
        "--output-lib", str(out_lib),
    ]
    if diann_report and Path(diann_report).exists():
        cmd += ["--diann-results", str(diann_report)]

    console.print(f"  Command: {' '.join(cmd)}\n")
    result = subprocess.run(cmd)
    if result.returncode == 0:
        console.print(f"\n[green]Library written:[/green] {out_lib}")
        console.print("Use this library in DIA-NN with:  --lib " + str(out_lib))
    else:
        console.print(f"\n[red]Carafe exited with code {result.returncode}.[/red]")
        raise typer.Exit(result.returncode)


@app.command("msbackend")
def msbackend(
    install: bool = typer.Option(False, "--install", help="Install MsBackendTimsTof and dependencies in R"),
    instrument: str = typer.Option(None, "--instrument", "-i", help="Filter .d files by instrument name"),
    output: Path = typer.Option(None, "--output", "-o", help="Write R script to this path instead of opening it"),
) -> None:
    """Generate an R script for TimsTOF raw data analysis via MsBackendTimsTof.

    MsBackendTimsTof is a Bioconductor/R package that reads Bruker .d directories
    as Spectra objects for deep MS data analysis — spectrum extraction, ion mobility
    access, filtering, and export.

    This command:
      1. Optionally installs R dependencies (--install, requires R on PATH)
      2. Queries the STAN database for all known .d file paths
      3. Writes a ready-to-run R script pre-filled with those paths
      4. Opens the script in the system default R/text editor
    ...
    if install:
        ...
        if not r_exec:
            console.print("[red]R not found on PATH.[/red] Install R from https://cran.r-project.org/ first.")
            raise typer.Exit(1)

        console.print("[bold]Installing MsBackendTimsTof R dependencies...[/bold]")
        console.print("  This may take several minutes on first install.\n")
...
# Install opentimsr from GitHub (required backend C++ library wrapper)
message("Installing opentimsr ...")
remotes::install_github("michalsta/opentims", subdir = "opentimsr", upgrade = "never")

# Install Spectra from Bioconductor
message("Installing Spectra ...")
BiocManager::install("Spectra", ask = FALSE, update = FALSE)

# Install MsBackendTimsTof from GitHub
message("Installing MsBackendTimsTof ...")
remotes::install_github("rformassspectrometry/MsBackendTimsTof", upgrade = "never")

message("\nDone! Run: stan msbackend  to generate your analysis script.")
message("NOTE: You must also download the Bruker proprietary library:")
message("  library(opentimsr)")
message("  opentimsr::download_bruker_proprietary_code('~/.stan/tools/bruker_sdk')")
message("  options(TIMSTOF_LIB = '~/.stan/tools/bruker_sdk/libtimsdata.so')")
"""
        r_tmp = Path(tempfile.mktemp(suffix=".R"))
        r_tmp.write_text(r_install_script)
        result = subprocess.run([r_exec, "--no-save", "--no-restore", str(r_tmp)],
                                text=True)
        r_tmp.unlink(missing_ok=True)
        if result.returncode != 0:
            console.print("[red]R installation failed.[/red] Check R output above.")
            raise typer.Exit(1)
        console.print("\n[green]R packages installed.[/green]")
        return

    # ── Gather .d paths from STAN DB ─────────────────────────────────
    runs = get_runs(instrument=instrument, limit=2000)
    d_paths = [
        r["raw_path"] for r in runs
        if r.get("raw_path") and r["raw_path"].endswith(".d")
    ]

    if not d_paths:
        console.print("[yellow]No .d file paths found in the STAN database.[/yellow]")
        console.print("Run [bold]stan baseline[/bold] or process some runs first.")
        d_paths_r = '  # no .d files in database yet'
    else:
        d_paths_r = "\n".join(f'  "{p}",' for p in d_paths)
        d_paths_r = d_paths_r.rstrip(",")
    ...
    r_script = f"""\
# ============================================================
# STAN-generated MsBackendTimsTof analysis script
# Generated: {__import__('datetime').datetime.now().strftime('%Y-%m-%d %H:%M')}
# Runs: {n} .d file(s){instr_label}
#
# Requirements:
#   1. R >= 4.1 with MsBackendTimsTof installed
#      Run: stan msbackend --install
#   2. Bruker proprietary SDK (libtimsdata.so / timsdata.dll)
#      Run from R:
#        library(opentimsr)
#        opentimsr::download_bruker_proprietary_code("~/.stan/tools/bruker_sdk")
#   3. Set the library path in your .Rprofile or before loading:
#      options(TIMSTOF_LIB = "~/.stan/tools/bruker_sdk/libtimsdata.so")
#
# Reference: https://github.com/rformassspectrometry/MsBackendTimsTof
# ============================================================

library(Spectra)
library(MsBackendTimsTof)

# -- Bruker SDK path (set this to your actual path) ---------------
options(TIMSTOF_LIB = path.expand("~/.stan/tools/bruker_sdk/libtimsdata.so"))
# On Windows: options(TIMSTOF_LIB = "C:/STAN/tools/bruker_sdk/timsdata.dll")

# -- .d file paths from STAN database -----------------------------
d_files <- c(
{d_paths_r}
)

# Filter to files that actually exist on this machine
d_files <- d_files[file.exists(d_files)]
if (length(d_files) == 0) stop("No .d files found — check paths above.")
message("Loading ", length(d_files), " TimsTOF run(s)...")

# -- Load spectra -------------------------------------------------
sps <- Spectra(d_files, backend = MsBackendTimsTof())
message("Loaded ", length(sps), " spectra across ", length(d_files), " run(s).")
...
cat("\\nAvailable variables:\\n")
print(spectraVariables(sps))

cat("\\nMS levels:\\n")
print(table(msLevel(sps)))

cat("\\nRetention time range (seconds):\\n")
print(range(rtime(sps)))

# -- Ion mobility (1/K0) ------------------------------------------
if ("ionMobility" %in% spectraVariables(sps)) {{
  cat("\\nIon mobility (1/K0) range:\\n")
  print(range(ionMobility(sps)))
}}

# -- Filter to MS1 spectra only -----------------------------------
ms1 <- filterMsLevel(sps, msLevel = 1L)
cat("\\nMS1 spectra:", length(ms1), "\\n")

# -- Filter to MS2 spectra only -----------------------------------
ms2 <- filterMsLevel(sps, msLevel = 2L)
cat("\\nMS2 spectra:", length(ms2), "\\n")

# -- Example: extract peaks from first MS2 spectrum ---------------
if (length(ms2) > 0) {{
  cat("\\nFirst MS2 spectrum peaks (m/z, intensity):\\n")
  pk <- peaksData(ms2[1])[[1]]
  print(head(pk, 10))
}}

# -- Example: filter by RT window (e.g., 5-10 min = 300-600 s) ---
# sps_rt <- filterRt(sps, rt = c(300, 600))

# -- Example: export peaks to data frame --------------------------
# peaks_df <- as.data.frame(peaksData(ms2))
"""
    ...
    if output:
        ...
        out_path = get_user_config_dir() / "msbackend_analysis.R"

    out_path.write_text(r_script, encoding="utf-8")
    console.print(f"[bold]R script written:[/bold] {out_path}")
    console.print(f"  Runs included: {n} .d file(s){instr_label}")
    console.print()
    console.print("Next steps:")
    console.print("  1. Install R packages (once):   [bold]stan msbackend --install[/bold]")
    console.print("  2. Download Bruker SDK (once):  see instructions inside the script")
    console.print(f"  3. Open in R/RStudio:           [bold]Rscript \"{out_path}\"[/bold]")
    console.print()

    # Try to open in system default editor
    r_exec = shutil.which("Rscript")
    rstudio = shutil.which("rstudio") or shutil.which("RStudio")
    if rstudio:
        subprocess.Popen([rstudio, str(out_path)])
        console.print(f"[green]Opened in RStudio.[/green]")
